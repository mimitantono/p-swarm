...
Bigmatrix::Bigmatrix(Db_data * db) {
	this->db = db;
	matrix = new bool*[db->sequences];
//	guestbook = new bool*[db->sequences];
...
//			if (guestbook[i] == 0 || !guestbook[i][j]) {
				if (qgram_diff(db->get_qgram_vector(i), db->get_qgram_vector(j)) <= Property::resolution) {
					...
					if (_result.diff <= Property::resolution) {
						if (matrix[i] == 0) {
							matrix[i] = new bool[db->sequences];
							for (unsigned long int k = 0; k < db->sequences; k++) {
								matrix[i][k] = false;
							}
						}
						matrix[i][j] = true;
						if (matrix[j] == 0) {
							matrix[j] = new bool[db->sequences];
							for (unsigned long int k = 0; k < db->sequences; k++) {
								matrix[j][k] = false;
							}
						}
...
//					if (guestbook[visited[k]] == 0) {
//						guestbook[visited[k]] = new bool[db->sequences];
//						for (unsigned long int m = 0; m < db->sequences; m++) {
//							guestbook[visited[k]][m] = false;
//						}
//					}
//					guestbook[visited[k]][visited[l]] = true;
//					if (guestbook[visited[l]] == 0) {
//						guestbook[visited[l]] = new bool[db->sequences];
//						for (unsigned long int m = 0; m < db->sequences; m++) {
//							guestbook[visited[l]][m] = false;
//						}
//					}
//					guestbook[visited[l]][visited[k]] = true;
//				}
//			}
//		}
		if (thread_id == 0)
...
bool Bigmatrix::has_match(int row_id) {
	for (unsigned long int j = row_id + 1; j < db->sequences; j++) {
		if (matrix[row_id] != 0 && matrix[row_id][j]) {
			return true;
		}
	}
	return false;
...
void Bigmatrix::crawl_row(bool ** row_guestbook, unsigned long int cluster_id, unsigned long int row_id, int generation) {
	...
	for (unsigned long int i = 0; i < db->sequences; i++) {
		if (!(*row_guestbook)[i] && matrix[row_id] != 0 && matrix[row_id][i]) {
			member_info member;
			member.sequence = *db->get_seqinfo(i);
